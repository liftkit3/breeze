...
1. **M1 S11** — Pause timer screen + completion flow (~2h). Replaces the `/pause/active/[id]` mock from S10.
2. **M1 S12** — Post-pause feedback screen (~1h)
3. **M1 S8** — Push notification infra (~3h, after manual loop validates)
4. **M1.5 polish bundle** — Apple Sign-In + Resend prod sender + Google consent branding + Domain router (S18-S21, deferred from S3/S4 pending Apple Developer + `breeze.app` purchase)

**Carry-over from S10** (small follow-ups, fold into S11 or a polish chore):
- Visual: button positioning at sheet bottom needed manual safe-area workaround. Proper `SafeAreaProvider` at root was reverted; revisit when polishing the picker visuals.
- DX: `apps/mobile/app/index.tsx` redirects unconditionally to `/(main)` (bypasses auth + onboarding). TODO marked in-file — revert before TestFlight.
- Spec deltas: drag-to-dismiss (no `react-native-gesture-handler` installed), custom `cubic-bezier` curve, and focus trap all deferred. Documented in `PickerSheet.tsx` header comment.
...
| 9 | Home screen | Pause-first explorer (HOME_FLOW spec): header + StreakChip, MoodCheckIn 4 valence levels (mal/ok/bien/genial) with AsyncStorage + midnight expiry, PauseHero with copy reactive to (mood × top hobby), 3-cell HobbiesGrid, floating glass BottomNav + coral FAB. Hero CTA + FAB share `/pause-coming-soon` stub until S10. YouTube long-pause rail intentionally omitted (no YT API key yet). | 3h | 1h | ✓ |
| 10a | LLM provider selection + billing | Provider chosen: Gemini 2.5 Flash via Google AI Studio (free tier, ~1500 req/day, no card). Considered Claude Haiku 4.5 (best ecosystem fit, no recurring free tier) and GPT-4o-mini (no clear win). Google AI Studio key created, `GEMINI_API_KEY` set as Supabase Edge secret, sanity-checked via throw-away `ping-gemini` function (200 OK, `modelVersion: gemini-2.5-flash`). Pre-launch flip to Vertex AI paid committed (free tier may train on prompts — not OK for mood data). CLAUDE.md + BACKLOG updated to reference Gemini. | 0.25h | 0.25h | ✓ |
| 10 | Pause Picker + Gemini API Edge Function | Picker bottom-sheet per `Stopit/PAUSE_PICKER_SPEC.md` (duration 5/10/15 + up to 3 hobby tiles + AI hint card + Empezar CTA), opens from coral FAB + hero CTA, replaces `/pause-coming-soon`. Edge Function `generate-activity` calls Gemini 2.5 Flash with `{mood, duration, hobby, recentFeedback[]}`, returns `{emoji, title, sub, motivation}` via structured JSON (`thinkingBudget: 0`). Client inserts `pauses` row with `status='pending'`, navigates to mocked `/pause/active/[id]`. Hobby keys read live from `profiles.hobbies[]` (custom strings → generic icon); 4-key mood (`bad/ok/good/great`) retained from S9; mood reactivity across screens via Zustand store (lifted from local useState). | 4h | 2h | ✓ |
...
| S10 | GEMINI_API_KEY (Google AI Studio, free tier ok para dev/M1 · flip a Vertex AI paid antes de M1.5 launch para evitar training-on-data) · key seteada como secret en Supabase Edge Functions ✓ (via S10a) |
...
[2026-05-13] [M1 S10] **Pause Picker + generate-activity Edge Function shipped.** Picker bottom-sheet renders per `Stopit/PAUSE_PICKER_SPEC.md`: title + sub, duration grid (5/10/15 with selected-state stone-900 fill), hobby grid (up to 3 from `profiles.hobbies[]`, custom strings fall back to `writing` icon, selected gets coral 2px border + halo shadow), AI hint card (sparkle icon + dynamic body composed from `{mood, duration, hobbyLabel}` using 4-key mood emoji map matching S9's MoodCheckIn), and coral pill CTA. Both entry points (`BottomNav` FAB + Home hero CTA) call `usePickerStore.open()`; sheet mounted in `(main)/_layout.tsx` so it overlays from anywhere in the main group. Edge Function `generate-activity` deployed to Supabase (project `xkzpehqgbrngkfyxaeju`), calls Gemini 2.5 Flash with structured JSON output (`responseSchema` enforces `{emoji, title, sub, motivation}`) and `thinkingConfig.thinkingBudget: 0` to skip the ~700-reasoning-token default. Auth-gated via Supabase user JWT. `usePauseSession` hook orchestrates: fetch last 5 `pauses.feeling_rating` rows for the user → invoke function → insert `pauses` row with `status='pending'` → navigate to `/pause/active/[id]`. **useMood migrated to Zustand store** (was local useState per consumer) so mood changes on Home reflect live in the Picker's AI hint card — single source of truth, AsyncStorage write/hydrate preserved, hook API unchanged. New tokens: `glass.tint.sheet` (0.85 white) and `glass.scrim.modal` (stone-900 0.18); `GlassSurface` gained `tint="sheet"` and `shadow="sheet"` (upward shadow) variants. `SparkleIcon` added to the icon registry. Active-pause screen `/pause/active/[id]` is a mock that renders title + sub + motivation + emoji + a "Volver al inicio" button — the real timer ships in S11. `pause-coming-soon.tsx` deleted. **Spec deltas (deferred, documented in PickerSheet.tsx header):** (1) Drag-to-dismiss skipped — `react-native-gesture-handler` not installed; scrim tap + Android back cover real dismiss paths. (2) Custom `cubic-bezier(0.2, 0.7, 0.2, 1)` enter curve replaced by RN Modal's default `animationType="slide"` — exact curve requires Reanimated + a custom Modal. (3) Focus trap skipped — RN Modal traps touches naturally; explicit VoiceOver focus management deferred. **Carry-over bugs:** button-position safe-area handling went through an iteration loop (SafeAreaProvider attempt reverted in favor of hardcoded `paddingBottom: 50` per founder call); revisit with proper `SafeAreaProvider` + `useSafeAreaInsets` in a polish chore. **DX hack:** `apps/mobile/app/index.tsx` redirects unconditionally to `/(main)`, bypassing auth + onboarding to speed up reload cycles during S10 build. TODO marked in-file — restore real gate before any preview/TestFlight build.

[2026-05-13] [M1 S10a] **LLM provider decided: Gemini 2.5 Flash via Google AI Studio.** Considered three frontier small-model options for the `generate-activity` Edge Function. Comparison at 700 tok/call, 150 calls/user/mo: Gemini Flash ~$0.015/user/mo, GPT-4o-mini ~$0.029, Claude Haiku 4.5 ~$0.225. All <2.3% of $9.99 revenue, so **cost was not the deciding factor at MVP scale**. Picked Gemini for the only criterion that mattered at this stage: **a real recurring free tier** — ~1500 req/day on Flash with no card, vs $5 one-time credits on Anthropic and OpenAI. Free tier covers dev + M1 internal TestFlight (300 active-user equivalent) without any spend. Setup (~15 min): Google AI Studio key created, `GEMINI_API_KEY` set as Supabase Edge secret (digest `62f292…`), throw-away `supabase/functions/ping-gemini/` deployed with `--no-verify-jwt`, curl returned 200 with `modelVersion: gemini-2.5-flash`. Function deleted from remote + local after verification. **Two follow-up commitments captured in CLAUDE.md HARD RULE:** (1) Before M1.5 launch with real users, **flip to paid Vertex AI** — AI Studio free tier may use prompts/responses to improve models, not acceptable for a wellness app touching mood data. (2) All `gemini-2.5-flash` calls must pass `thinkingConfig: { thinkingBudget: 0 }` — smoke test showed the default model burns ~686 reasoning tokens before generating a 5-token response, wasted on creative short outputs and ~7× more expensive once on paid tier. Doc updates: CLAUDE.md HARD RULE renamed "Claude API → LLM API (Gemini)" with the provider decision + thinking-budget note inline; `CLAUDE_API_KEY` → `GEMINI_API_KEY` globally; pre-flight external-services list updated; `supabase/functions/generate-activity/` tree comment updated. S10 acceptance + external prereqs row rewritten to reflect Gemini + the Picker spec (`Stopit/PAUSE_PICKER_SPEC.md`).
